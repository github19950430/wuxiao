...
(function (window, undefined) {
    ...
    function set_task_speed(task, total, loaded) {
        ...
        //计算剩余时间
        task.remainTimes = Math.round((total-loaded) / task.speed);
    }
    ...
    */
    ...
    Uploader.prototype = {
        ...
        set: function (settings) {
            ...
            self.checkMd5 = ops.checkMd5 == undefined || typeof ops.checkMd5 !== 'function' ? function(resp){return false;} : ops.checkMd5;//验证MD5是否正确
        ...
        addTask: function (input, file) {
            ...
            if ((self.disallows && self.disallows[ext]) || (self.allows && !self.allows[ext])) {
            	if(typeof QError ==="function"){
            		QError("ext");
            	}
            	console.warn("上传的文件类型限制");
            	limit_type = "ext";
            }
            else if (size != -1 && self.maxSize && size > self.maxSize) {
            	console.warn("上传的文件大小超出了限制");
            	limit_type = "size";
            }
        ...
        queryState: function (task, callback) {
            ...
            var postQueryStr = "";
            var postData = self.data;
            for(var key in postData){
            	postQueryStr += "&"+encodeURIComponent(key) + "=" + encodeURIComponent(postData[key]);
            }
            task.queryUrl = url + (url.indexOf("?") == -1 ? "?" : "&") + "action=query&hash=" + (task.hash || encodeURIComponent(task.name)) + "&fileName=" + encodeURIComponent(task.name) + (task.size != -1 ? "&fileSize=" + task.size : "") + postQueryStr;
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    if (json.ret === 1 || self.checkMd5(responseText)) {
                        task.queryOK = true;
