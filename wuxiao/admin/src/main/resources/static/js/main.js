layui.use(['element', 'form', 'laydate', 'layer', 'upload'], function () {
    ...
    var laydate = layui.laydate; //加载laydate模块
    
    echoImg();

    //仅用于一个页面一个日期插件(多个日期插件请将此放置页面或该页面附属js)
    laydate.render({
		elem : '.laydate',
	});
    ...
    $(".layui-side").on("click", function () {
        ...
        if (to.attr("toggle") === 'on') {
            ...
            $('.mytree .layui-nav-item').each(function(index,e){
            	if($(this).hasClass('layui-nav-itemed')){//展开的
//            		$(this).find('.lia').css({background:'#f64945'})
            	}else{
//            		$(this).find('.lia').css({background:'#f64945'})
//            		$(this).find('.lia').css({background:'#db2c28'})
            	}
            })
 ...
 // 多条数据修改--后台处理
    $(".more_param").on("click", function (e) {
        e.preventDefault();
            ...
            if (msg !== undefined) {
                layer.confirm(msg + '？', {
                    ...
                    btn: ['确认', '取消']
                	 ...
                	 $.get(url, checked.join("&"), function (result) {
                         $.fn.Messager(result);
                     });
                });
            } else {
