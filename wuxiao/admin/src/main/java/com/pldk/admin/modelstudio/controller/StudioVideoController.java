...
import com.pldk.admin.modelstudio.validator.StudioVideoValid;
 ...
 */
@Controller
@RequestMapping("/modelstudio/studioVideo")
public class StudioVideoController {

    @Autowired
    private StudioVideoService studioVideoService;

    /**
     * 设置一条或者多条数据的状态
     */
    @RequestMapping("/status/{param}")
    @RequiresPermissions("modelstudio:studioVideo:status")
    @ResponseBody
    public ResultVo status(
        ...
        if (studioVideoService.updateStatus(statusEnum, ids)) {
