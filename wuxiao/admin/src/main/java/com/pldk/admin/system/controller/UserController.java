...
public class UserController {
    ...
    public ResultVo save(@Validated UserValid valid, @EntityParam User user) {
        ...
        // 保存数据
        //user.setUnitType(UnitTypeEnum.LABORUN_UNIT_TYPE.getCode());
        int roleId=0;
        if(user.getUnitType()==UnitTypeEnum.LABORUN_UNIT_TYPE.getCode()) {
        	 userService.save(user);
        	 return ResultVoUtil.SAVE_SUCCESS;
        }
        if(user.getUnitType()==UnitTypeEnum.GROUP_UNIT_TYPE.getCode()) {
        	//团省委
        	roleId=5;
        }
        if(user.getUnitType()==UnitTypeEnum.SCIENCE_UNIT_TYPE.getCode()) {
        	//省科协
        	roleId=7;
        }
    	// 保存数据
