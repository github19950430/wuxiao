...
//            判断评审专家是否有效
            if(null!=user.getIsExpert()&&1==user.getIsExpert()&&!StringUtils.isEmpty(user.getExpEndDate())){
                String nowDate= DateUtil.getTime();
                if(nowDate.compareTo(user.getExpEndDate()+" 23:59:59")>-1){
                    return ResultVoUtil.error("该账号已失效,请联系管理员重新激活！");
                }
            }
